...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if ((n1 + n2) % 2 == 0) {
            return ((double) helper(nums1, 0, n1 - 1, nums2, 0, n2 - 1, (n1 + n2) / 2) + (double) helper(nums1, 0, n1 - 1, nums2, 0, n2 - 1, (n1 + n2) / 2 + 1)) / 2;
        }

        return (double) helper(nums1, 0, n1 - 1, nums2, 0, n2 - 1, (n1 + n2) / 2 + 1);
    }

    private int helper(int[] nums1, int s1, int e1, int[] nums2, int s2, int e2, int k) {
        if (k == 1) {
            int first1 = s1 <= e1 ? nums1[s1] : Integer.MAX_VALUE;
            int first2 = s2 <= e2 ? nums2[s2] : Integer.MAX_VALUE;
            return Math.min(first1, first2);
        }

        int mid1 = s1 + k / 2 - 1 <= e1 ? nums1[s1 + k / 2 - 1] : Integer.MAX_VALUE;
        int mid2 = s2 + k / 2 - 1 <= e2 ? nums2[s2 + k / 2 - 1] : Integer.MAX_VALUE;

        if (mid1 < mid2) {
            return helper(nums1, s1 + k / 2, e1, nums2, s2, e2, k - k / 2);
        }

        return helper(nums1, s1, e1, nums2, s2 + k / 2, e2, k - k / 2);
    }
}